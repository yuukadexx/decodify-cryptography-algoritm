...
from models import db, User, UserProgress, CipherUsage, UserActivity, QuizQuestion, Challenge, Achievement, GlossaryTerm, GlossaryCategory, UserBookmark, SearchHistory, QuizAttempt, ChallengeAttempt
...
def tutorials():
    ...
    if current_user.is_authenticated:
        ...
        return render_template('tutorials.html',
                             ...
                             user_xp=progress.xp,
                             is_premium=False)  # Anda bisa tambahkan logika premium
    else:
        return render_template('tutorials.html',
                             username='Guest',
                             user_level=1,
...
@app.route('/tutorials/klasik')
def tutorials_klasik():
    return render_template('tutorial_category.html', 
                          category='Cipher Klasik',
                          description='Cipher bersejarah dari Caesar, Vigenère, Playfair hingga Enigma Machine')

@app.route('/tutorials/modern')
def tutorials_modern():
    return render_template('tutorial_category.html', 
                          category='Kriptografi Modern',
                          description='RSA, AES, Hash Functions, Diffie-Hellman, dan kriptografi asimetris modern')

@app.route('/tutorials/cryptanalysis')
def tutorials_cryptanalysis():
    return render_template('tutorial_category.html', 
                          category='Kriptanalisis',
                          description='Teknik memecahkan cipher: Frequency Analysis, Brute Force, dan Side-Channel attacks')

@app.route('/tutorials/applications')
def tutorials_applications():
    return render_template('tutorial_category.html', 
                          category='Aplikasi Praktis',
                          description='Implementasi dalam SSL/TLS, Blockchain, Password Security, dan Messaging Apps')

@app.route('/tutorials/advanced')
@login_required
def tutorials_advanced():
    """Halaman tutorial lanjutan - hanya untuk premium users"""
    progress = get_user_progress(current_user.id)
    return render_template('tutorial_advanced.html',
                         username=current_user.username,
                         user_level=progress.level,
                         user_xp=progress.xp)

# Route untuk detail tutorial
@app.route('/tutorial/<tutorial_id>')
def tutorial_detail(tutorial_id):
    """Halaman detail tutorial"""
    tutorial_data = get_tutorial_data(tutorial_id)  # Fungsi helper yang perlu Anda buat
...
def api_get_glossary_terms():
    """Get paginated glossary terms with filters"""
    try:
        ...
        if search:
            ...
            query = query.filter(
                db.or_(
                    GlossaryTerm.title.ilike(search_term),
                    GlossaryTerm.definition.ilike(search_term),
                    GlossaryTerm.tags.ilike(search_term)
                )
            )
        ...
        if letter and letter != 'all':
            query = query.filter_by(letter=letter.upper())
        
        # Apply sorting
        if sort_by == 'title':
            if sort_order == 'asc':
                query = query.order_by(GlossaryTerm.title.asc())
            else:
                query = query.order_by(GlossaryTerm.title.desc())
        elif sort_by == 'views':
            query = query.order_by(GlossaryTerm.view_count.desc())
        elif sort_by == 'newest':
            query = query.order_by(GlossaryTerm.created_at.desc())
        elif sort_by == 'featured':
            query = query.order_by(GlossaryTerm.is_featured.desc(), GlossaryTerm.title.asc())
        ...
        terms_data = []
        for term in terms:
            tags_list = term.tags.split(',') if term.tags else []
            ...
            terms_data.append({
                ...
                'letter': term.letter,
                'year': term.year,
                'inventor': term.inventor,
                'application': term.application,
                'example': term.example,
                'footnote': term.footnote,
                'tags': tags_list,
                'view_count': term.view_count,
                'bookmarked_count': term.bookmarked_count,
                'is_featured': term.is_featured,
        ...
        category_counts = {}
        difficulty_counts = {
            'beginner': GlossaryTerm.query.filter_by(difficulty='beginner').count(),
            'intermediate': GlossaryTerm.query.filter_by(difficulty='intermediate').count(),
            'advanced': GlossaryTerm.query.filter_by(difficulty='advanced').count()
        }
        
        # Get letter counts
        letter_counts = {}
        for letter_char in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ':
            count = GlossaryTerm.query.filter_by(letter=letter_char).count()
            if count > 0:
                letter_counts[letter_char] = count
        ...
        return jsonify({
            ...
            'statistics': {
                ...
                'featured_count': GlossaryTerm.query.filter_by(is_featured=True).count()
            },
            'filters': {
                'search': search,
                'category': category,
        ...
        print(f"Error in glossary terms API: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/glossary/terms/<int:term_id>', methods=['GET'])
def api_get_glossary_term(term_id):
    """Get detailed information about a specific glossary term"""
    try:
        ...
        if tags_list:
            ...
            for tag in tags_list[:3]:  # Limit to first 3 tags
                related = GlossaryTerm.query.filter(
                    GlossaryTerm.tags.ilike(f'%{tag}%'),
                    GlossaryTerm.id != term.id
                ).limit(3).all()
                
                for rel in related:
                    if rel not in related_terms:
                        related_terms.append(rel)
                
                if len(related_terms) >= 5:  # Limit total related terms
                    break
        
        # If not enough related terms, get terms from same category
        if len(related_terms) < 3:
            same_category = GlossaryTerm.query.filter(
                GlossaryTerm.category == term.category,
                GlossaryTerm.id != term.id
            ).limit(5 - len(related_terms)).all()
            related_terms.extend(same_category)
        ...
        # Prepare related terms data
        related_terms_data = []
        for rel in related_terms[:5]:  # Limit to 5 related terms
            related_terms_data.append({
                'id': rel.id,
                'title': rel.title,
                'definition': rel.definition[:150] + '...' if len(rel.definition) > 150 else rel.definition,
        ...
        return jsonify({
            'success': True,
            'term': term_data,
            'related_terms': related_terms_data
        })
...
@app.route('/api/glossary/search/quick', methods=['GET'])
def api_quick_search():
    """Quick search for glossary terms (for autocomplete)"""
    try:
        ...
        results = GlossaryTerm.query.filter(
            db.or_(
                GlossaryTerm.title.ilike(search_term),
                GlossaryTerm.definition.ilike(search_term),
                GlossaryTerm.tags.ilike(search_term)
            ...
            db.case(
                (GlossaryTerm.title.ilike(f"{query}%"), 0),  # Exact start match first
                (GlossaryTerm.title.ilike(search_term), 1),   # Then other title matches
                else_=2                                       # Then other matches
            )
        ).limit(limit).all()
        ...
        for term in results:
            ...
            title = term.title
            definition_preview = term.definition[:100] + '...' if len(term.definition) > 100 else term.definition
            ...
            results_data.append({
                ...
                'difficulty': term.difficulty,
                'letter': term.letter,
                'icon': get_category_icon(term.category)
            })
...
def api_get_categories():
    ...
    try:
        categories = GlossaryCategory.query.filter_by(is_active=True).order_by(GlossaryCategory.display_order).all()
        
        categories_data = []
        for cat in categories:
            # Get term count for this category
            term_count = GlossaryTerm.query.filter_by(category=cat.name).count()
...
def api_get_glossary_stats():
    """Get glossary statistics"""
    try:
        total_terms = GlossaryTerm.query.count()
        total_categories = GlossaryCategory.query.filter_by(is_active=True).count()
        featured_terms = GlossaryTerm.query.filter_by(is_featured=True).count()
        
        # Get most viewed terms
        most_viewed = GlossaryTerm.query.order_by(GlossaryTerm.view_count.desc()).limit(5).all()
        most_viewed_data = [{
            'id': term.id,
            'title': term.title,
            'views': term.view_count,
            'category': term.category
        } for term in most_viewed]
        
        # Get most bookmarked terms
        most_bookmarked = GlossaryTerm.query.order_by(GlossaryTerm.bookmarked_count.desc()).limit(5).all()
        most_bookmarked_data = [{
            'id': term.id,
            'title': term.title,
            'bookmarks': term.bookmarked_count,
            'category': term.category
        } for term in most_bookmarked]
        
        # Get recent terms
        recent_terms = GlossaryTerm.query.order_by(GlossaryTerm.created_at.desc()).limit(5).all()
        recent_terms_data = [{
            'id': term.id,
            'title': term.title,
            'created_at': term.created_at.isoformat() if term.created_at else None,
        ...
        category_stats = []
        categories = GlossaryCategory.query.filter_by(is_active=True).all()
        for cat in categories:
            count = GlossaryTerm.query.filter_by(category=cat.name).count()
            if count > 0:
                category_stats.append({
                    'name': cat.name,
                    'count': count,
                    'percentage': round((count / total_terms * 100), 1) if total_terms > 0 else 0
                })
        ...
        return jsonify({
            ...
            'most_bookmarked': most_bookmarked_data,
            'recent_terms': recent_terms_data
        })
...
@app.route('/api/glossary/bookmark', methods=['POST'])
@login_required
def api_toggle_bookmark():
    """Toggle bookmark for a glossary term"""
    try:
        data = request.get_json()
        ...
        term = GlossaryTerm.query.get(term_id)
        if not term:
        ...
        if bookmark:
            ...
            term.bookmarked_count = max(0, term.bookmarked_count - 1)
            action = 'removed'
            is_bookmarked = False
        else:
            # Add bookmark
        ...
        activity = UserActivity(
            ...
            description=f'{action.capitalize()} bookmark for "{term.title}"',
            cipher_name='glossary'
        )
        db.session.add(activity)
        db.session.commit()
        ...
        return jsonify({
            ...
            'action': action,
            'is_bookmarked': is_bookmarked,
            'bookmark_count': term.bookmarked_count,
...
@app.route('/api/glossary/bookmarks', methods=['GET'])
@login_required
def api_get_bookmarks():
    ...
    try:
        ...
        bookmarks = UserBookmark.query.filter_by(user_id=current_user.id)\
            .join(GlossaryTerm)\
...
@app.route('/api/glossary/history', methods=['GET'])
@login_required
...
@app.route('/api/glossary/featured', methods=['GET'])
def api_get_featured_terms():
    """Get featured glossary terms"""
    try:
        limit = request.args.get('limit', 6, type=int)
        
        featured_terms = GlossaryTerm.query.filter_by(is_featured=True)\
            .order_by(GlossaryTerm.created_at.desc())\
            .limit(limit)\
            .all()
        
        terms_data = []
        for term in featured_terms:
            terms_data.append({
                'id': term.id,
                'title': term.title,
...
@app.route('/api/glossary/random', methods=['GET'])
def api_get_random_term():
    """Get a random glossary term"""
    try:
        # Get a random term
        term = GlossaryTerm.query.order_by(db.func.random()).first()
...
def get_category_icon(category_name):
    ...
    icon_map = {
        'Kriptografi Modern': 'microchip',
        'Kriptografi Klasik': 'landmark',
        'Matematika Kripto': 'calculator',
        'Protokol Kripto': 'handshake',
        'Kriptanalisis': 'crosshairs',
        'Hash Functions': 'fingerprint',
        'Digital Signatures': 'signature',
        'Kriptografi Kuantum': 'atom',
        'Blockchain Crypto': 'link',
        'Kriptografi Asimetris': 'key',
        'Kriptografi Simetris': 'key',
        'Steganografi': 'eye-slash'
    }
    return icon_map.get(category_name, 'book')
...
def init_glossary_data():
    ...
    if GlossaryCategory.query.count() == 0:
        categories = [
            ...
            {'name': 'Matematika Kripto', 'icon': 'calculator', 'description': 'Konsep matematika dalam kriptografi'},
            {'name': 'Protokol Kripto', 'icon': 'handshake', 'description': 'Protokol keamanan dan komunikasi'},
            {'name': 'Kriptanalisis', 'icon': 'crosshairs', 'description': 'Teknik analisis dan pemecahan cipher'},
            {'name': 'Hash Functions', 'icon': 'fingerprint', 'description': 'Fungsi hash dan aplikasinya'},
            {'name': 'Digital Signatures', 'icon': 'signature', 'description': 'Tanda tangan digital dan autentikasi'},
            {'name': 'Kriptografi Kuantum', 'icon': 'atom', 'description': 'Kriptografi berbasis mekanika kuantum'},
            {'name': 'Blockchain Crypto', 'icon': 'link', 'description': 'Kriptografi dalam teknologi blockchain'},
            {'name': 'Kriptografi Asimetris', 'icon': 'key', 'description': 'Kriptografi kunci publik'},
            {'name': 'Kriptografi Simetris', 'icon': 'key', 'description': 'Kriptografi kunci rahasia'},
            {'name': 'Steganografi', 'icon': 'eye-slash', 'description': 'Penyembunyian informasi'},
        ]
    ...
    if GlossaryTerm.query.count() == 0:
        terms = [
            {
                ...
                'pronunciation': '/ˈzɪər.oʊ ˈnɒl.ɪdʒ pruːf/',
                'definition': 'Protokol kriptografi di mana satu pihak (prover) dapat membuktikan kepada pihak lain (verifier) bahwa suatu pernyataan benar, tanpa mengungkap informasi apa pun selain kebenaran pernyataan tersebut.',
                'category': 'Protokol Kripto',
                'difficulty': 'advanced',
                'letter': 'Z',
                'year': '1985',
                'inventor': 'Shafi Goldwasser, Silvio Micali, Charles Rackoff',
                'application': 'Authentication, Blockchain (Zcash), Privacy-preserving verification',
                'example': 'Dalam ZK-SNARKs (Zero-Knowledge Succinct Non-Interactive Argument of Knowledge), transaksi blockchain dapat divalidasi tanpa mengungkap pengirim, penerima, atau jumlah.',
                'footnote': 'Konsep fundamental dalam kriptografi modern yang memungkinkan privasi tanpa mengorbankan keamanan.',
                'tags': 'ZK-SNARKs,ZK-STARKs,Bulletproofs,Cryptographic Proofs',
                'is_featured': True
            },
            {
                'title': 'Homomorphic Encryption',
                'pronunciation': '/ˌhɒm.əˈmɔː.fɪk ɪnˈkrɪp.ʃən/',
                'definition': 'Bentuk enkripsi yang memungkinkan komputasi pada data terenkripsi tanpa perlu mendekripsinya terlebih dahulu. Hasil komputasi, setelah didekripsi, sama dengan hasil operasi yang dilakukan pada data asli.',
                'category': 'Kriptografi Modern',
                'difficulty': 'advanced',
                'letter': 'H',
                'year': '1978',
                'inventor': 'Ronald Rivest, Len Adleman, Michael Dertouzos',
                'application': 'Secure cloud computing, Private data analysis, Medical research',
                'example': 'Seorang peneliti dapat menganalisis data medis terenkripsi di cloud tanpa pernah melihat data pasien secara langsung.',
                'footnote': 'Masih menjadi area penelitian aktif dengan trade-off antara keamanan, efisiensi, dan fungsionalitas.',
                'tags': 'Fully Homomorphic Encryption,Partially Homomorphic,Lattice-based Crypto',
                'is_featured': True
            },
            {
                'title': 'Post-Quantum Cryptography',
                'pronunciation': '/pəʊst ˈkwɒn.təm krɪpˈtɒɡ.rə.fi/',
                'definition': 'Algoritma kriptografi yang diyakini aman terhadap serangan oleh komputer kuantum. Juga dikenal sebagai kriptografi tahan kuantum.',
                'category': 'Kriptografi Kuantum',
                'difficulty': 'advanced',
                'letter': 'P',
                'year': '1990s',
                'inventor': 'Multiple researchers',
                'application': 'Future-proof encryption, Government communications, Critical infrastructure',
                'example': 'NIST telah mengstandardisasi algoritma seperti CRYSTALS-Kyber (KEM) dan CRYSTALS-Dilithium (signatures) sebagai standar PQC.',
                'footnote': 'Transisi ke PQC diperlukan sebelum komputer kuantum skala besar menjadi realitas.',
                'tags': 'Lattice-based,Code-based,Multivariate,Hash-based',
                'is_featured': True
            },
            # Tambahkan lebih banyak terms sesuai kebutuhan
            {
                'title': 'Elliptic Curve Cryptography',
                'pronunciation': '/ɪˈlɪp.tɪk kɜːrv ˌkrɪpˈtɒɡ.rə.fi/',
                'definition': 'Pendekatan kriptografi kunci publik berdasarkan struktur aljabar kurva eliptik di atas bidang hingga. Menawarkan keamanan yang setara dengan RSA dengan kunci yang lebih pendek.',
                'category': 'Kriptografi Modern',
                'difficulty': 'intermediate',
                'letter': 'E',
                'year': '1985',
                'inventor': 'Neal Koblitz, Victor S. Miller',
                'application': 'SSL/TLS, Bitcoin/Blockchain, Secure messaging',
                'example': 'Bitcoin menggunakan kurva eliptik secp256k1 untuk menghasilkan pasangan kunci publik-privat wallet.',
                'footnote': '256-bit ECC setara dengan 3072-bit RSA dalam hal keamanan.',
                'tags': 'ECDSA,ECDH,EdDSA,Curve25519'
            },
            {
                'title': 'Differential Cryptanalysis',
                'pronunciation': '/ˌdɪf.əˈren.ʃəl ˌkrɪp.təˈnæl.ə.sɪs/',
                'definition': 'Metode kriptanalisis yang mempelajari bagaimana perbedaan dalam input mempengaruhi perbedaan output dalam cipher, untuk menurunkan kunci rahasia.',
                'category': 'Kriptanalisis',
                'difficulty': 'advanced',
                'letter': 'D',
                'year': '1990',
                'inventor': 'Eli Biham, Adi Shamir',
                'application': 'Breaking block ciphers, Cipher design analysis',
                'example': 'Metode ini digunakan untuk menyerang DES dengan mengurangi kompleksitas dari 2^56 menjadi 2^47.',
                'footnote': 'Penemuan ini mengubah paradigma desain cipher blok modern.',
                'tags': 'Linear Cryptanalysis,Boomerang Attack,Impossible Differential'
            },
            {
                'title': 'Merkle Tree',
                'pronunciation': '/ˈmɜːr.kəl triː/',
                'definition': 'Struktur data pohon di mana setiap node non-leaf adalah hash dari node anaknya. Digunakan untuk verifikasi data yang efisien dan aman.',
                'category': 'Hash Functions',
                'difficulty': 'intermediate',
                'letter': 'M',
                'year': '1979',
                'inventor': 'Ralph Merkle',
                'application': 'Blockchain, Git, Certificate transparency, P2P networks',
                'example': 'Bitcoin menggunakan Merkle Tree untuk merangkum semua transaksi dalam sebuah blok, memungkinkan verifikasi ringan (light clients).',
                'footnote': 'Dasar dari banyak protokol keamanan modern dan struktur data terdistribusi.',
                'tags': 'Merkle Proof,Merkle Patricia Tree,Hash Tree'
            },
            {
                'title': 'Shamir\'s Secret Sharing',
                'pronunciation': '/ˈʃæ.mɪrz ˈsiː.krət ˈʃer.ɪŋ/',
                'definition': 'Skema berbagi rahasia kriptografi yang membagi sebuah rahasia menjadi beberapa bagian, di mana hanya subset tertentu yang cukup untuk merekonstruksi rahasia asli.',
                'category': 'Protokol Kripto',
                'difficulty': 'intermediate',
                'letter': 'S',
                'year': '1979',
                'inventor': 'Adi Shamir',
                'application': 'Key management, Multi-signature wallets, Distributed trust',
                'example': 'Sebuah kunci enkripsi dapat dibagi menjadi 5 bagian, di mana 3 bagian cukup untuk merekonstruksi kunci asli.',
                'footnote': 'Berdasarkan interpolasi polinomial di atas bidang hingga.',
                'tags': 'Threshold Cryptography,Multi-party Computation,Secret Splitting'
            },
            {
                'title': 'Lamport Signature',
                'pronunciation': '/ˈlæm.pɔːrt ˈsɪɡ.nə.tʃər/',
                'definition': 'Skema tanda tangan digital satu kali berdasarkan fungsi hash. Merupakan skema tanda tangan kuantum-aman yang pertama.',
                'category': 'Digital Signatures',
                'difficulty': 'intermediate',
                'letter': 'L',
                'year': '1979',
                'inventor': 'Leslie Lamport',
                'application': 'Post-quantum signatures, One-time authentication',
                'example': 'Digunakan dalam protokol blockchain tertentu yang membutuhkan tanda tangan tahan kuantum.',
                'footnote': 'Membutuhkan kunci yang sangat besar tetapi sederhana dan aman secara konseptual.',
                'tags': 'Merkle Signature,Hash-based Signatures,Winternitz OTS'
            }
        ]
        ...
        for term_data in terms:
            term = GlossaryTerm(
                ...
                letter=term_data['letter'],
                year=term_data.get('year'),
                inventor=term_data.get('inventor'),
                application=term_data.get('application'),
                example=term_data.get('example'),
                footnote=term_data.get('footnote'),
                tags=term_data.get('tags'),
                is_featured=term_data.get('is_featured', False)
            )
            db.session.add(term)
        
        db.session.commit()
        print(f"✅ {len(terms)} istilah glossary ditambahkan")
        
        # Update category counts
        for category in GlossaryCategory.query.all():
            count = GlossaryTerm.query.filter_by(category=category.name).count()
            category.term_count = count
        
        db.session.commit()
    
    print("✅ Data glossary berhasil diinisialisasi")
...
def initialize_database():
    ...
    init_glossary_data()  # Tambahkan ini
